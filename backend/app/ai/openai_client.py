...
def answer_token_budget(question: str) -> int:
    ...
    if any(marker in normalized for marker in detailed_markers):
        return min(
            320,
            settings.ollama_generation_num_predict * 2,
        )
    ...
    return min(
        96,
        settings.ollama_generation_num_predict,
    )
    ...
    return [
        {
            ...
            "content": (
                ...
                "Give complete steps or detail only when requested. "
                "Do not repeat the question, evidence, introductions, "
                "or conclusions."
            ),
        },
        {
...
def _record_ollama_metadata(response: dict) -> None:
    ...
    profiler.set(
        **values,
        ollama_prompt_eval_count=response.get(
            "prompt_eval_count"
        ),
...
async def stream_answer(
    ...
    """Stream answer text from the configured LLM provider."""
    profiler = get_query_profiler()
    ...
    try:
        ...
        if provider == "groq":
            stream = _stream_groq(question, context)

        elif provider == "ollama":
            stream = _stream_ollama(question, context)
        ...
        logger.exception(
            "Groq returned an error. Status: %s",
            exc.status_code,
        )
...
async def generate_answer(
    ...
    try:
        ...
        if provider == "groq":
            answer = await _generate_groq(
                question,
                context,
            )

        elif provider == "ollama":
            answer = await _generate_ollama(
                question,
                context,
            )
        ...
        if not answer:
            logger.error(
                "%s returned a response without answer content.",
                provider,
            )
        ...
        if exc.status_code == 404:
            raise AIServiceError(
                ...
                f"'{settings.groq_model}' is not available."
            ) from exc
        ...
        logger.exception(
            "An HTTP error occurred while calling Ollama."
        )
        raise AIServiceError(
            "The AI service is currently unavailable."
        ) from exc
